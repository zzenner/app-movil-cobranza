...
| RF-07g | **Gestión administrativa de usuarios — escritura (Fase 5B-2 ✅):** El administrador puede crear usuarios (`POST /api/v1/admin/usuarios`), editar datos básicos (`PUT .../datos-basicos`), activar, desactivar, bloquear, desbloquear y restablecer la contraseña. La creación asigna roles iniciales; no hay endpoint para modificar roles post-creación. El sistema protege al último administrador activo (HTTP 409 `ULTIMO_ADMINISTRADOR` si la operación dejaría el sistema sin admin). El actor no puede desactivarse ni bloquearse a sí mismo (HTTP 409 `AUTO_LOCKOUT`). Las operaciones de seguridad (desactivar, bloquear, restablecer-contraseña) revocan todas las sesiones activas del target de forma atómica. Locking optimista via campo `version`. BCrypt: se valida que la contraseña no supere 72 bytes UTF-8 antes de hashear. Permiso requerido: `USUARIOS_ADMINISTRAR`. Ver RN-29–35, ADR-0047, ADR-0048. |
